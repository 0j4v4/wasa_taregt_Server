...
class motor_driver:
    ...
    def Status(self):
        status = yaml.load(self.jrk2cmd('-d', self.serialnumber, '-s', '--full'), Loader=yaml.FullLoader)
    ...
    def GetPosition(self):
        while True:
            time.sleep(1);
            currentPos = yaml.load(self.jrk2cmd('-d', self.serialnumber, '-s', '--full'), Loader=yaml.FullLoader)["Scaled feedback"]
